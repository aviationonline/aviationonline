...
export default function Dashboard() {
  ...
  const handleResetPasswordEmail = async () => {
    if (!profile?.email) {
      showNotification(language === 'en' ? "Error: User email not found." : "Erreur : E-mail utilisateur introuvable.", 'error');
      return;
    }
    ...
    try {
      ...
      showNotification(
        language === 'en' 
          ...
          : "Un lien de réinitialisation de mot de passe a été envoyé sur votre adresse e-mail.", 
        "success"
      );
    } catch (e: any) {
      console.error("Failed to send reset email from dashboard:", e);
      showNotification(
        language === 'en'
          ? "Failed to send reset link: " + (e.message || e)
          : "Erreur lors de l'envoi du lien : " + (e.message || e), 
        "error"
      );
    } finally {
      ...
      <header className="mb-8 md:mb-12 flex flex-col md:flex-row md:items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button 
            ...
            className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-zinc-600 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-all disabled:opacity-50"
          >
            <Key className="w-4 h-4" />
            {sendingResetEmail ? "Envoi en cours..." : "Changer de mot de passe"}
          </button>
          <button 
